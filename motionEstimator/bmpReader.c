...
void DCT(BMPData *image, float **dct_Y, float **dct_U, float **dct_V, unsigned char ** Y, unsigned char ** U, unsigned char ** V, int blockSize, int dSampleHeight, int dSampleWidth)
{
	...
	float cu,cv;
	if(dSampleWidth == 0 || dSampleHeight == 0)
	{
			...
			for (u = 0; u < blockSize; ++u)
			{
				...
				for (v = 0; v < blockSize; ++v)
				{
					...
					for (x = 0; x < blockSize; ++x)
					{
						...
						{
							if(i == 0 && j == 0)
								printf("!DCT-->value:%f u:%d v:%d x:%d y:%d dct_u:%d c1:%f c2:%f\n",value1,u,v,x,y,U[x + i][y + j],cos((pi *(2*x + 1)*u)/16),cos((pi *(2*y + 1)*v)/16));
...
void IDCT(BMPData *image, int **dct_Y, int **dct_U, int **dct_V, unsigned char ** Y, unsigned char ** U, unsigned char ** V, int blockSize, int dSampleHeight, int dSampleWidth)
{
	...
	int **DCT_U;
	int **DCT_V;
	DCT_U = (int **) malloc(sizeof(int *) * dSampleHeight);
	DCT_V = (int **) malloc(sizeof(int *) * dSampleHeight);
	for (i = 0; i < dSampleHeight; ++i)
	{
		DCT_U[i] = (int*) malloc(sizeof(int) * dSampleWidth);
		DCT_V[i] = (int*) malloc(sizeof(int) * dSampleWidth);
	}
	for (i = 0; i < dSampleHeight; ++i)
	{
		for (j = 0; j < dSampleWidth; ++j)
		{
			 DCT_U[i][j] = dct_U[i][j];
			 DCT_V[i][j] = dct_V[i][j];
		}
			...
			for (x = 0; x < blockSize; ++x)
			{
				for (y = 0; y < blockSize; ++y)
				{
					...
					// printf("x:%d y:%d value:%f \n",x+i,y+j,round(0.25*value));
					 if(round(0.25*value1) > 255) 
					 	printf("YGreater--> i: %d j: %d value1:%f \n",i,j,value1);
					if(round(0.25*value) > 255)
						Y[x + i][y + j] = 255;
					else if(round(0.25*value) < 0)
						Y[x + i][y + j] = 0;
					else
						Y[x + i][y + j] = (unsigned char) abs(round(0.25*value));// / luminanceQuantizationMatrix[u][v];	
			...
			for (x = 0; x < blockSize; ++x)
			{
				for (y = 0; y < blockSize; ++y)
				{
					...
					for (u = 0; u < blockSize; ++u)
					{
						...
						for (v = 0; v < blockSize; ++v)
						{
							holder = DCT_U[u + i][v + j];
							if(v == 0)
								cv = 1/sqrt(2);
							else
								cv = 1;
							if(i == 0 && j == 0)
								printf("IDCT--> value: %f u:%d v:%d x:%d y:%d dct_u[%d][%d]:%d c1:%f c2:%f const:%d const:%d \n",value1,u,v,x,y,u + i,v + j,dct_U[u + i][v + j],cos((pi *(2*x + 1)*u)/16),cos((pi *(2*y + 1)*v)/16),DCT_U[0][0],holder);
					 ...
					 if(round(0.25*value1) > 255) 
					 	printf("Greater--> i: %d j: %d value1:%f dctU[0][0]: %d\n",i,j,value1,dct_U[0][0]);
					// 	U[x + i][y + j] = 255;
					// else if(round(0.25*value1) < 0)
					// 	U[x + i][y + j] = 0;
					// else 
						U[x + i][y + j] = (unsigned char) abs(round(0.25*value1));
						if(i == 0 && j == 0)
							printf("pixel:%d \n",U[x + i][y + j]);
					// if(round(0.25*value2) > 255)	
					// 	V[x + i][y + j] = 255;
					// else if(round(0.25*value2) < 0)
					// 	V[x + i][y + j] = 0;
					// else
						V[x + i][y + j] = (unsigned char) abs(round(0.25*value2));
				}
			}
