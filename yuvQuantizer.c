...
void quantiser(unsigned char ** Y,unsigned char ** U,unsigned char ** V,int height, int width, int mask)
{
	int i,j;
...
int main(int argc, char const *argv[])
{
	if(argc != 3)
	{
		printf("\n\nThe format is ./quantizer [file name] [quantizer Mask 00-FF]\n\n");
		return -1;
	}

	int i,ERROR;
	int quantMask = atoi(argv[2]);
	...
	quantiser(Y,U,V,infoHeader.width,infoHeader.height,quantMask);
